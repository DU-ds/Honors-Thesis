...
for(i in 1:R){
 ...
 dv.m1.partyid[j,i]<- cor(v[,L-1],v[,j], use="na.or.complete")
  }
}
...
library(magrittr)
to.upper.by.row <- function(X) t(X)[lower.tri(X,diag=F)]
m1.type <- c(rep(2,14),rep(3,17),rep(5,10),rep(7,6))
...
matrix.m1.partyid  <- cbind(dv.m1.partyid, m1.time, m1.type)
matrix.m1.ideology <- cbind(dv.m1.ideology, m1.time,m1.type)
...
matrix.m1.ideology[,1] -> dv.i
matrix.m1.ideology[,2] -> t.i
matrix.m1.ideology[,3] -> type.i
plot(dv.i ~ t.i) 
...
summary(e1)
coef(e1)
#see how the multilevel anova shows good results
#now do this for other issue domains?
...
plot(dv.pi~ t.i)
abline(reg = lm(dv.pi~ t.i))
plot(dv.ip~t.i)
abline(reg = lm(dv.ip~ 0 + t.i))
m.ip <-  lm(dv.ip~ 0 + t.i)
summary(m.ip)
range(t.i)
m.ip2 <- lm(dv.ip ~ 0 + t.i + type.i)
summary(m.ip2)
#that's interesting, I expected the sign to be negative on type.i because
#type.i is going from econ -> civ rights -> moral-> foreign policy.
#I expected econ to increase. Maybe it's bc it's the proportion? No, even
#still, I expected the proportion of econ to increase more than the others.
plot(dv.i ~ dv.p)


library(ggplot2)
#install.packages("reshape2")
#run this code above iff you never installed the package
library(reshape2)
length(dv.i)  == length(dv.p)
#so the vector of types for either will work for this.
df <- cbind(dv.i, dv.p , dv.ip, type.i, t.i)
df <- as.data.frame(df)
ggplot(data = df, aes(x = t.i , y = dv.ip ), color = type.i )+
geom_smooth()
library(dplyr)

 df3 <- df3 %>% ddply( )%>% group_by(V3)) 

range(df3$V3

breakdown <- function(x,y,logi){ #x needs to be a data.frame or array
 for(i in seq_along(x)){ # logi is a logical operator
 	 list(out, typeof(x))
 	 out[i] <- x[x logi y[i]] #y needs to be a vector or list
 }
}
