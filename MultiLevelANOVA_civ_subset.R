...
vars.m2c <- cbind(anes.12$affirmative.action, anes.12$fair.treat.blacks, anes.12$blacks.aid, anes.12$civ.rights.too.fast, anes.12$equal.opportunities, anes.12$blacks.try.harder, anes.12$no.favor.blacks, anes.12$blacks.deserve.more, anes.12$hard.for.blacks, anes.12$school.busing, anes.12$school.integr, anes.12$negro.pos.changed, anes.12$more.eq.chances, anes.12$too.much.eq.rights, anes.12$less.equality, anes.12$equal.chances, anes.12$equal.treatment, anes.12$year_sequence)
#civil rights subset of vars.m2c 

library(magrittr)
year <- anes.12$`Year of study` %>% as.factor() %>% levels() %>% as.numeric()
year2 <- year - range(year)[[1]]
decade80 <- year2 - median(year2)
#centered at the median


L2 <- dim(vars.m2c)[2]
R <- range(vars.m2c[,L2])[2]
cor.array.m2 <- c(rep(NA,((L2-1)^2 * R)))
dim(cor.array.m2) <- c(L2-1,L2-1,R)
for(i in 1:R) {
  print(i)
  v <- subset(vars.m2c, vars.m2c[,L2] == i)
  print(dim(v))
  v <- v[,-c(L2)]
  print(dim(v))
