...
#vars.m2 <- cbind(anes.12$FS.poor, anes.12$FS.welfare, anes.12$FS.aid.college, anes.12$FS.public.schools, anes.12$FS.aids, anes.12$FS.childcare, anes.12$FS.food.stamps, anes.12$FS.envir, anes.12$FS.soc.security, anes.12$FS.homeless, anes.12$FS.assist.blacks, anes.12$health.insurance, anes.12$jobs.guarantee, anes.12$gov.services, anes.12$year_sequence) 
#econ subset of vars.m2  
#anes.12$affirmative.action, anes.12$fair.treat.blacks, anes.12$blacks.aid, anes.12$civ.rights.too.fast, anes.12$equal.opportunities, anes.12$blacks.try.harder, anes.12$no.favor.blacks, anes.12$blacks.deserve.more, anes.12$hard.for.blacks, anes.12$school.busing, anes.12$school.integr, anes.12$negro.pos.changed, anes.12$more.eq.chances, anes.12$too.much.eq.rights, anes.12$less.equality, anes.12$equal.chances, anes.12$equal.treatment, anes.12$year_sequence
#vars.m2 <- cbind(anes.12$gay.military, anes.12$homosex.protect, anes.12$gay.children, anes.12$moral.behavior, anes.12$different.values, anes.12$traditional.values, anes.12$new.lifestyles, anes.12$women.role, anes.12$abortion, anes.12$school.prayer, anes.12$year_sequence
#vars.m2 <- cbind(anes.12$defense.spending, anes.12$urb.unrest, anes.12$ussr.cooperation, anes.12$FS.foreign.aid, anes.12$FS.space, anes.12$FS.crime anes.12$year_sequence)
...
for(i in 1:R){
 ...
 dv.m1.partyid[j,i]<- cor(v[,L-1],v[,j], use="na.or.complete") #L-1 is party id, j itterates over issue 1:47
  }
}

dv.m1.partyid <- as.vector(dv.m1.partyid)
...
for(i in 1:R){
 ...
 for(j in 1:L3){print(j)
 dv.m1.ideology[j,i]<- cor(v[,L-1],v[,j], use="na.or.complete")
  }
}

dv.m1.ideology <- as.vector(dv.m1.ideology)
...
for(i in 1:R) {
  #print(i) #used for diagnostics in writing the code
  v <- subset(vars.m2, vars.m2[,L2] == i)
  #print(dim(v)) #used for diagnostics in writing the code
  v <- v[,-c(L2)]
  #print(dim(v)) #used for diagnostics in writing the code
...
#type <- c(rep(1,14),rep(2,17),rep(3,10),rep(4,6))
#This vector(should be one that) gives the type of each column in the vars.partyid & vars.ideology matrices.
c(47:1) -> q
...
#check that it ran correctly; if it didn't, fix it so it does

m2.type <- m2.type1 * m2.type2
#check that it gives outputs for 
#"1x1 <- 4, 1x2 <- 6,1x3 <- 10, 1x4 <- 14, 2x2 <- 9,
#2x3 <- 15, 2x4 <- 21, 3x3 <- 25, 3x4 <- 35, 4x4 <- 49 
#for all i & j in 1:4, i *j == j * i" to all be enumerated!


ROW1 <- c(1:47)
m2issuei <- ROW1 %>% rep(length(ROW1)) %>% matrix(ncol =  length(ROW1), byrow = F) %>% to.upper.by.row()
m2issuej <- ROW1 %>% rep(length(ROW1)) %>% matrix(ncol =  length(ROW1), byrow = T) %>% to.upper.by.row()
# issue i indexes the row
# issue j indexes the column



#year_indicator <- c(rep(NA, dim(cor.column.vectors.partyid)[1] * dim(cor.column.vectors.partyid)[2]))
#year_indicator.matrix <- matrix(year_indicator, ncol = dim(cor.column.vectors.partyid)[2])

  
matrix.m1.partyid  <- cbind(dv.m1.partyid, m1.time, m1.type, qp)
matrix.m1.ideology <- cbind(dv.m1.ideology, m1.time,m1.type, qp)
matrix.m2 <- cbind(dv.m2, m2.time, m2.type, m2.type1, m2.type2, m2issuei, m2issuej) 
...
mp.t1 <- lmer(dv.p ~ 1 + t.p + (1 + t.p | ip)) #model from table one, p for party id
mp.t2 <- lmer(dv.p ~ 1 + t.p * type.p + (1 + t.p | ip)) #model from table two, p for party id
mp.lm1 <- lm(dv.p ~ t.p + ip)
mp.lm2 <- lm(dv.p ~ t.p + type.p + ip)
summary(mp.lm1)
summary(mp.lm2)
plot(mp.lm1, which = c(1:6)) #there are 6 plots according to the R docs
plot(mp.lm2, which = c(1:6)) #not sure which of the 6 is the residual vs order plot tbh 
...
mi.t1 <- lmer(dv.i ~ 1 + t.i + (1 + t.i | ii)) #model from table one, i for ideology 
mi.t2 <- lmer(dv.i ~ 1 + t.i * type.i + (1 + t.i | ii)) #model from table two, i for ideology 
mi.lm1 <- lm(dv.i ~ t.i + ii)
mi.lm2 <- lm(dv.i ~ t.i + type.i + ii)
summary(mi.lm1)
summary(mi.lm2)

plot(mi.lm1, which = c(1:6)) # looking for serial correlation
plot(mi.lm2, which = c(1:6)) # which would show up in an order vs residual plot in a lm


#the m2 stuff didn't get off the ground. I ran out of time tbh :(
#update 04/15/18, I'm going to figure it out bc it's really important empirically
...
dv2      <- matrix.m2[,1]
t2       <- matrix.m2[,2]
type2    <- matrix.m2[,3] %>% as.factor()
type2row <- matrix.m2[,4] %>% as.factor()
type2col <- matrix.m2[,5] %>% as.factor()
issuei   <- matrix.m2[,6]
issuej   <- matrix.m2[,7]

LM1 <- lm(dv2 ~  t2  )
LM2 <- lm(dv2 ~  t2  + issuej + issuei)
anova(LM1, LM2)
Anova(LM1)
Anova(LM2)
M2 <- lmer(dv2 ~  t2  + (t2 | issuej) + (t2 | issuei))
#show Reuben and ask him to help me see if it's significant.
#It should be significant if the no pooling and full pooling is significant, right?

#install.packages("car") #uncomment if the next line gives an error
library("car")
Anova(M2)
...
#difference is I specified uncorrelated slope and intercept terms while B & G
#specified correlated slops and intercept terms
#the fit models are almost identical
...
#residual analysis
plot(m.p1@resp$wtres~ m.p1@resp$y)
abline(reg = lm(m.p1@resp$wtres~ m.p1@resp$y))
res_pid <- lm(m.p1@resp$wtres~ m.p1@resp$y)
