...
vars.m2m <- cbind(anes.12$gay.military, anes.12$homosex.protect, anes.12$gay.children, anes.12$moral.behavior, anes.12$different.values, anes.12$traditional.values, anes.12$new.lifestyles, anes.12$women.role, anes.12$abortion, anes.12$school.prayer, anes.12$year_sequence)
#moral subset of vars.m2m 
library(magrittr)
...
mor1 <- lmer(dv2 ~ 1 + (1 |t2))
summary(mor1)
coef(mor1)

mor1coef <- coef(mor1)$t2
class(mor1coef)
